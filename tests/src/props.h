...
public:
    ...
    LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
    {
        ...
        ATL::CWindow btnClose = sheet.GetDlgItem(IDOK);
        ...
        LPCTSTR lpstrCompression = _T("Uncompressed");;
        ...
        if(m_lpstrFilePath != NULL)
        {
            ...
            if(hFile != INVALID_HANDLE_VALUE)
            {
                ...
                SetDlgItemInt(IDC_WIDTH   , (UINT)bih.biWidth);
                SetDlgItemInt(IDC_HEIGHT  , (UINT)bih.biHeight);
                SetDlgItemInt(IDC_HORRES  , (UINT)::MulDiv(bih.biXPelsPerMeter, 254, 10000));
                SetDlgItemInt(IDC_VERTRES , (UINT)::MulDiv(bih.biYPelsPerMeter, 254, 10000));
        ...
        else        // must be pasted from the clipboard
        {
            ...
            {
                CClientDC dc(NULL);
                SetDlgItemInt(IDC_WIDTH       , (UINT)bitmap.bmWidth);
                SetDlgItemInt(IDC_HEIGHT      , (UINT)bitmap.bmHeight);
                // should we use screen resolution here???
                SetDlgItemInt(IDC_HORRES      , (UINT)dc.GetDeviceCaps(LOGPIXELSX));
                SetDlgItemInt(IDC_VERTRES     , (UINT)dc.GetDeviceCaps(LOGPIXELSX));
...
public:
    ...
    LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
    {
        ...
        CClientDC dc(NULL);
        SetDlgItemInt(IDC_WIDTH   , (UINT)dc.GetDeviceCaps(HORZRES));
        SetDlgItemInt(IDC_HEIGHT  , (UINT)dc.GetDeviceCaps(VERTRES));
        SetDlgItemInt(IDC_HORRES  , (UINT)dc.GetDeviceCaps(LOGPIXELSX));
        SetDlgItemInt(IDC_VERTRES , (UINT)dc.GetDeviceCaps(LOGPIXELSY));
        SetDlgItemInt(IDC_BITDEPTH, (UINT)dc.GetDeviceCaps(BITSPIXEL));
